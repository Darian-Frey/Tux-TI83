...
The UI redesign (CLAUDE.md task queue) just finished. Immediate next-up
work is **reintegrating the legacy features** (graph mode, matrix editor,
history pane, popups) into the new component-based UI, then **filling in
the no-op CalcKeys** (2ND, MODE, ALPHA, MATH, x², Ans).
...
- 📅 Audit and remove legacy [graph_ui/qml/Main.qml](graph_ui/qml/Main.qml) once everything useful is ported into the new UI
...
### Basic arithmetic
- ✅ `+ − × ÷`
- ✅ Power `^` *(but right-associativity is broken — see [BUG-009](BUGS.md))*
- ✅ Square root `√(` *(silent failure on negatives — see [BUG-006](BUGS.md))*
...
### Transcendental
- ✅ `sin`, `cos`, `tan`
- 📅 `asin`, `acos`, `atan` — parser accepts them but the evaluator is a no-op; see [BUG-004](BUGS.md)
- ✅ `log` (base 10), `ln`
...
- ✅ `and`, `or`, `not` (engine + UI)
- ✅ `xor` (engine implemented; same — needs UI exposure)
- 📅 Logic operator menu in the new UI (the legacy popup needs porting)
...
- ✅ Determinant `det(` (engine fully implemented via recursive Laplace expansion; needs a UI button to be reachable from the keypad)
- ✅ Registry `[A]`, `[B]`, `[C]` (UI exposure — engine actually supports `[A]`–`[J]`, just needs more entries in the controller's token table)
- ✅ 3×3 matrix editor (legacy popup — needs porting to new UI)
- 🚧 Reintegrate matrix editor into the new component-based UI
- 📅 Matrix subtraction — see [BUG-008](BUGS.md) (trivial fix, just mirror `matrixAdd`)
- 📅 Transpose `T`
- 📅 Inverse `^-1`
...
- ✅ Multi-function plot (Y1, Y2, Y3)
- ✅ Pan (click-drag) and zoom (scroll wheel)
- ✅ ZStandard, Zoom Fit
- 🚧 Reintegrate graph mode into the new component-based UI (legacy Main.qml has it but isn't loaded anymore)
- 🔜 Wire the soft-key row (`Y=`, `WINDOW`, `ZOOM`, `TRACE`, `GRAPH`)
...
### Window settings
- ✅ Xmin/Xmax/Ymin/Ymax (legacy WINDOW popup, exists but not in new UI)
- 🚧 Reintegrate WINDOW popup into the new UI
- 📅 Xscl/Yscl (axis tick spacing)
- 📅 Xres (graph resolution / x-step)
...
- ✅ `CalcKey`, `Display`, `SoftKeyRow` reusable components
...
### Up next
- 🔜 Wire CalcKeys: 2ND, MODE, ALPHA, MATH, x², Ans (currently no-op)
- 🔜 Reintegrate the history pane in the new UI
...
- 📅 WINDOW popup ported to the new UI
- 📅 Matrix editor popup ported to the new UI
- 📅 Logic operator menu ported to the new UI

### Considering
- 💭 Resizable window (currently fixed 420×760)
