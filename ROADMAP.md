...
- 🚧 Distributions — **normal family done** (`normalpdf(`, `normalcdf(`, `invNorm(`), landed 2026-07-26. Optional μ/σ (default 0/1) padded to fixed arity by `rewriteDistCalls` (recurses for nested calls); `normalcdf` via `std::erf`, `invNorm` via Acklam's inverse-normal approximation. `σ>0` and `invNorm` area ∈ (0,1) domain-checked. In the MATH menu. Still 📅: discrete (`binompdf(`, `binomcdf(`, `poissonpdf(`, `poissoncdf(`, `geometpdf(`, `geometcdf(`) and continuous (`tpdf(`, `tcdf(`, `χ²pdf(`, `χ²cdf(`, `Fpdf(`, `Fcdf(`).
