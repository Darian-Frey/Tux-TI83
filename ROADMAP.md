...
- 📅 `MODE` → **Screen: Full / Horiz / G-T** (split-screen) — Full is the current behaviour. Horiz = graph over a home-entry strip; G-T = graph beside a table. Needs a split-screen layout: the main view is a `StackLayout` swapping keypad ↔ graph ↔ table, so a split would show two pages at once. G-T (graph + table, both already full components) is the cleaner fit; Horiz is awkward because our "home" page is the keypad, not a home-entry strip. Deferred as a focused layout task.
