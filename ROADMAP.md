...
- ✅ List ↔ Matrix conversion — `List▶Matr(L1,…,Ln)` (variadic, n columns) / `Matr▶List([A],col)` value-producing forms landed 2026-08-01 (see the Matrices section). ✅ `Matr▶List` all-column split added 2026-08-14 (see the Matrices section).
- 💭 Custom named lists (`αLIST` / arbitrary names beyond `L1`–`L6`) — **deferred** (2026-08-14). The engine only knows the fixed `L1`–`L6` tokens, so arbitrary names need a whole controller-level layer (name→values storage, store interception, per-expression name resolution, list-editor UI, persistence) for low marginal value. Its own increment if ever wanted.
...
- ✅ Remaining 2ND variants — complete 2026-08-13. TEST menu, insert-mode toggle, CATALOG (2ND+0), nth-root (2ND+^), `2ND+(`/`)` → `{`/`}`, and `L1`–`L6` on `2ND+1..6` were already wired; **EE / scientific-exponent entry** landed now as `2ND+,` → inserts `×10^` (e.g. `1.5×10^3` = 1500; `×`/`^` are existing tokens so `core_math` is untouched — `^` binds tighter than `×`). The comma key gained an `ᴇ` 2ND label, and the physical keyboard now maps `,` (so `For(I,1,5)` is typeable and `\`+`,` gives EE). Remaining 2ND functions without a dedicated key are reachable via CATALOG, matching the real TI-83. 3 tests.
