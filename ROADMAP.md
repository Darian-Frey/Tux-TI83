...
- ✅ Matrix ↔ List conversion (`List▶Matr`, `Matr▶List`) — value-producing binary forms matching the engine's value+STO idiom (added 2026-08-01). `List▶Matr(Lα,Lβ)` → n×2 matrix with each list a column (equal lengths else `ERR:INVALID DIM`; non-list arg → `ERR:DATA TYPE`); `Matr▶List([A],col)` → 1-based column as a list (out-of-range col → `ERR:INVALID DIM`). Store results with `→[C]` / `→Ln`. Engine + UI via MatrixPopup MATH tab. 📅 still: the variadic TI forms (`List▶Matr` with 3+ columns, `Matr▶List` splitting all columns at once) — need variadic-arg parsing + baked-in store-target semantics.
...
- ✅ List ↔ Matrix conversion — `List▶Matr(Lα,Lβ)` / `Matr▶List([A],col)` value-producing binary forms landed 2026-08-01 (see the Matrices section for full semantics). Variadic TI forms (3+ columns) still 📅.
...
- 🚧 `MODE` menu follow-ups — Angle, Notation (Normal/Sci/Eng), Decimal (Float/Fix N), Base (Dec/Hex/Oct/Bin), Draw (Connected/Dot), **Graph: Func/Par/Pol/Seq (all four wired)**, and **Complex: Real/a+bi/re^θi (wired)** are done — Graph rows back `setGraphMode`, Complex backs `setComplexMode`, both are live READ/WRITE/NOTIFY properties the header indicator also binds to (verified 2026-08-01). Remaining placeholders, still deliberately greyed until their features exist: Plot: Sequential/Simul (needs a frame-by-frame animation framework), Screen: Full/Horiz/G-T (needs split-screen layout). Each needs a backing property + renderer support.
